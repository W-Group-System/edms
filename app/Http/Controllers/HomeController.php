...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $months = [];
        $company_policies = Document::with('department','change_requests')->whereIn('category',['POLICY','PROCEDURE','DEPARTMENT MANUAL','ANNEX','WORK INSTRUCTION'])->whereNull('status')->get();
        ...
        if((auth()->user()->role != "Administrator") || (auth()->user()->role != "Management Representative") || (auth()->user()->role != "Business Process Manager"))
        {
            if((auth()->user()->role == "Department Head"))
            {
                ...
                $company_policies = Document::with('department')->whereIn('department_id',(auth()->user()->accountable_persons)->pluck('department_id')->toArray())->whereIn('category',['POLICY','PROCEDURE','DEPARTMENT MANUAL','ANNEX','WORK INSTRUCTION'])->where('status',null)->get();
            ...
            elseif((auth()->user()->role == "Document Control Officer"))
            {
                ...
                $company_policies = Document::with('department')->whereIn('department_id',(auth()->user()->dco)->pluck('department_id')->toArray())->whereIn('category',['POLICY','PROCEDURE','DEPARTMENT MANUAL','ANNEX','WORK INSTRUCTION'])->where('status',null)->get();
            }
