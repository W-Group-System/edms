...
@section('content')
                        ...
                        <div class="ibox-tools" style="float: none;">
                            <form method="GET" action="" id="filterForm" style="display: inline-block; width: 150px;">
                                <select name="status_filter"
                                    ...
                                    <option value="Cancelled" {{ request('status_filter') == 'Cancelled' ? 'selected' : '' }}>
                                        Cancelled
                                    </option>
                                </select>
                            </form>
                                    ...
                                    @foreach ($account_requests as $account_request)
                                        ...
                                        @if(request('status_filter'))
                                            @php
                                                ...
                                                if ($filter == 'Cancelled' && $account_request->status != 'Cancelled') {
                                                    continue;
                                                }
                                            @endphp
                                        @endif
                                        ...
                                        <tr>
                                            <td>
                                                @if(($account_request->status != 'Approved') && $account_request->status != 'Declined')
                                                    <button type="button" class="btn btn-sm btn-danger cancelAccountRequest" id="{{ $account_request->id }}">
    ...
    @include('account_request.new_account_requests')
@endsection
