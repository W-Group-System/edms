    ...
    <div class="modal-dialog modal-lg " role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class='col-md-10'>
                    <h5 class="modal-title" id="exampleModalLabel">Edit DCO</h5>
                </div>
                <div class='col-md-2'>
                        ...
                        <div class='col-md-12'>
                            ...
                            <select name='department[]' class='form-control-sm form-control cat' multiple required>
                                ...
                                @foreach($departments->where('status',null) as $dep)
                                    <option value='{{$dep->id}}' @if(count(($user->dco)->where('department_id',$dep->id)) == 1) selected @endif>{{$dep->code}} - {{$dep->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
